...
- [x] `AltTextGenerator.tsx` uses the deprecated `document.execCommand("copy")` instead of the existing `copyToClipboard` helper (`src/lib/utils.ts`), which uses the modern Clipboard API. Switch it over. — Done (PR #13): `AltTextGenerator.tsx` now calls `copyToClipboard`. Turned out `navigator.clipboard` isn't available in Figma's plugin UI iframe, so `copyToClipboard` itself tries the Clipboard API first and falls back to `execCommand("copy")` internally — the deprecated call still exists, but it's now an encapsulated fallback in one shared helper instead of being reimplemented ad hoc in the component.
...
- [ ] Add a test suite (vitest) for the pure logic that currently has zero coverage: `getContrastCompliance`, `isTouchTargetTooSmall`/`isTouchTargetTooClose`, background-colour inference. These are the functions whose correctness the whole plugin's credibility rests on. — In progress: vitest + jsdom are now set up (`vitest.config.ts`, `npm run test`) with a first suite for `copyToClipboard` (`src/lib/utils.test.ts`), but the specific functions named here still have zero coverage.

...
export default async function generateAltTextForLayer() {
	...
	if ("width" in selectedNode && "height" in selectedNode) {
		if (selectedNode.width <= 0 || selectedNode.height <= 0) {
			figma.notify("Selected layer has invalid dimensions and cannot be exported.");
			return;
		}
	...
	try {
		...
		if (payloadSizeMB > 10) {
			figma.notify("Image is too large for processing. Please select a smaller element.");
			return;
		}

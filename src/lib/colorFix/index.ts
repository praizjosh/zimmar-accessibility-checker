...
function walkLightnessToward(
	...
	const searchAtChroma = (chromaScale: number): RGBColor | null => {
		const atPole = achieves(withChroma({ ...originalLab, l: targetPole }, chromaScale));
		...
		for (let i = 0; i < MAX_ITERATIONS; i++) {
			const mid = (lo + hi) / 2;
			const midRatio = achieves(withChroma({ ...originalLab, l: mid }, chromaScale));
...
export function suggestAccessibleColor(
	...
	const requiredRatio = getRequiredRatio(targetLevel, isLargeText(fontSize, isBold));

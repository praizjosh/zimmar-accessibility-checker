...
describe("suggestAccessibleColor", () => {
	it("returns null when the pair is already compliant at the requested level", () => {
		expect(suggestAccessibleColor(BLACK, WHITE, 16, false, "AA", "foreground")).toBeNull();
	});

	it("darkens text that's the lighter of the two colours", () => {
		const foreground: RGBColor = [128, 128, 128];
		const result = suggestAccessibleColor(foreground, WHITE, 16, false, "AA", "foreground");
		...
		expect(getContrastCompliance(result!.color, WHITE, 16, false).compliance).not.toBe("Fail");
	});
	...
	it("lightens text that's already the darker of the two colours, rather than darkening it further", () => {
		...
		expect(getContrastCompliance(result!.color, background, 16, false).compliance).not.toBe(
			"Fail",
		);
	});
	...
	it("adjusts the background instead of the text when direction is 'background'", () => {
		...
		expect(getContrastCompliance(foreground, result!.color, 16, false).compliance).not.toBe(
			"Fail",
		);
	});
	...
	it("targets a stricter ratio for AAA than for AA", () => {
		...
		const aaResult = suggestAccessibleColor(foreground, WHITE, 16, false, "AA", "foreground");
		const aaaResult = suggestAccessibleColor(foreground, WHITE, 16, false, "AAA", "foreground");
	...
	it("uses the lower large-text threshold once fontSize crosses 18px", () => {
		...
		const normal = suggestAccessibleColor(foreground, WHITE, 17, false, "AA", "foreground");
		// At/above 18px: large text, only needs 3:1 for AA - a smaller lightness
		// change should suffice, so the two results should differ.
		const large = suggestAccessibleColor(foreground, WHITE, 18, false, "AA", "foreground");
	...
	it("treats bold text at 14px as large text, same as the shared getContrastCompliance logic", () => {
		const foreground: RGBColor = [150, 150, 150];
		const boldLarge = suggestAccessibleColor(foreground, WHITE, 14, true, "AA", "foreground");
	...
	it("returns null when no in-gamut candidate exists in either direction", () => {
		...
		expect(getContrastCompliance(WHITE, background, 16, false).ratio).toBeLessThan(7);
		expect(getContrastCompliance(BLACK, background, 16, false).ratio).toBeLessThan(7);

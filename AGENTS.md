...
- **Run an accessibility review before every commit or push, not just at PR-review time.** Before committing or pushing any piece of work, run an accessibility pass (WCAG 2.1/2.2 AA, via the `web-accessibility` agent) alongside the usual correctness/logic check — this repo's whole purpose is accessibility checking, so a UI/UX regression here is especially costly to ship unnoticed. Treat it as a required step, the same as running lint/tests, not an optional extra.
